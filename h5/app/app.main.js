(function () {
	...
	AppController.$inject = ['$scope', '$location', '$localStorage', '$window', '$rootScope', '$state', '$controller', 'CONSTANTS', 'API', 'ENUM', 'ngToast', 'CartModel', 'AppAuthenticationService'];

	function AppController($scope, $location, $localStorage, $window, $rootScope, $state, $controller, CONSTANTS, API, ENUM, ngToast, CartModel, AppAuthenticationService) {
		...
		if (query) {
			...
			if(weixinToken && weixinToken.length > 0){
				AppAuthenticationService.setToken(weixinToken);
			}

			if(weixinOpenId && weixinOpenId.length > 0){
